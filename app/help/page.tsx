    ...
    <div className="min-h-screen bg-[#F8F8F6] dark:bg-[#09090B] text-[#0D0D0F] dark:text-[#F4F4F5] font-sans antialiased flex flex-col justify-between transition-colors duration-200">
      ...
      <main className="max-w-[1280px] mx-auto px-4 md:px-8 py-10 md:py-16 w-full flex-1">
        ...
        <div className="space-y-6 mb-16 max-w-4xl">
          <h2 className="text-xl font-extrabold text-[#0D0D0F] dark:text-[#F4F4F5]">Frequently Asked Questions</h2>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {faqs.map((item, idx) => (
              <div key={idx} className="bg-white dark:bg-[#18181B] p-6 rounded-2xl border border-[#E5E7EB] dark:border-[#27272A] shadow-xs space-y-2">
                <h3 className="text-sm font-bold text-[#0D0D0F] dark:text-[#F4F4F5]">{item.q}</h3>
                <p className="text-xs text-[#6E7280] dark:text-[#A1A1AA] leading-relaxed">{item.a}</p>
              </div>
            ))}
          </div>
        </div>
